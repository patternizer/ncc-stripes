...
subset_labels = {annotation_labels[date] for date in annotation_dates if (date >= t_start) & (date <= t_end)}
subset_dates = {annotation_dates[date] for date in annotation_dates if (date >= t_start) & (date <= t_end)}
...
if plot_climate_timeseries == True:
    ...
    figstr = 'climate-timeseries' + '-' + projectionstr + '-' + smoothstr + baselinestr + '-' + 'rebinning' + '.png'
    ...
    if use_overlay_annotations == True:
        if use_log10_scale == False:
            ...
            for i in range(len(subset_labels)): 
                ...
                plt.axvline(x=list(subset_dates)[i], ls='dashed', lw=1, color='grey')
                plt.text( list(subset_dates)[i], ylimits[0]-2.5, list(subset_labels)[i], weight='bold', color='grey' )    
...
if plot_climate_bars == True:
    ...
    figstr = 'climate-bars' + '-' + projectionstr + '-' + smoothstr + baselinestr + '-' + 'rebinning' + '.png'
    titlestr = 'Global mean anomaly, 65 Myr (<2015) - 2200 CE: ' + projectionstr
...
# PLOT: ( 2A ) climate bars 65 Myr ( < 2015 ) - 500 CE ------------------------
    ...
    figstr = 'climate-bars-panel-A' + '-' + smoothstr + baselinestr + '-' + 'rebinning' + '.png'
...
#    if use_norwich_era_colormap == True: colors, norm, sm = rescale_colormap(y[-1])    
    if use_norwich_era_colormap == True: colors, norm, sm = rescale_colormap( cbar_max_norwich_era )    
    ...
    figstr = 'climate-bars-panel-B' + '-' + projectionstr + '-' + smoothstr + baselinestr + '-' + 'rebinning' + '.png'
...
if plot_climate_stripes == True:
    ...
    figstr = 'climate-stripes' + '-' + projectionstr + '-' + smoothstr + baselinestr + '-' + 'rebinning' + '.png'
    titlestr = 'Global mean anomaly, 65 Myr (<2015) - 2200 CE: ' + projectionstr
...
# PLOT: ( 3A ) climate stripes 65 Myr ( < 2015 ) - 500 CE ------------------------
    ...
    figstr = 'climate-stripes-panel-A' + '-' + smoothstr + baselinestr + '-' + 'rebinning' + '.png'
    titlestr = 'Global mean anomaly, 65 Myr (<2015) - 500 CE'
...
#    if use_norwich_era_colormap == True: colors, norm, sm = rescale_colormap(y[-1])    
    ...
    figstr = 'climate-stripes-panel-B' + '-' + projectionstr + '-' + smoothstr + baselinestr + '-' + 'rebinning' + '.png'
    titlestr = 'Global mean anomaly, 500-2200 CE: ' + projectionstr
    ...
    if use_overlay_colorbar == True:    
        ...
        cbar.set_label( cbarstr, rotation=270, labelpad=25, fontsize=fontsize )
    if use_overlay_timeseries == True: 
        plt.plot( np.arange( len(p1x)+len(p2x)+len(p3x)+len(p4x), len(x) ), Y_norm, color='black', ls='-', lw=1 )        
    if use_overlay_annotations == True:

...
if projectionstr == 'SSP119':
    projectionstr_title = 'SSP1-1.9'
elif projectionstr == 'SSP126':
    projectionstr_title = 'SSP1-2.6'
elif projectionstr == 'SSP245':
    projectionstr_title = 'SSP2-4.5'
elif projectionstr == 'SSP370':
    projectionstr_title = 'SSP3-7.0'
elif projectionstr == 'SSP585':
    projectionstr_title = 'SSP5-8.5'
...
titlestr = 'Global mean anomaly, 1-2100 CE: ' + projectionstr_title + ' forecast'
...
x = np.array( list(p5x) + list(p6x) + list(p7x) )
y = np.array( list(p5y) + list(p6y) + list(p7y) )
...
if plot_climate_bars == True:
    ...
    figstr = 'climate-bars' + '-' + projectionstr + '-' + baselinestr + '-' + 'full_multiaxis' + '.png'
          
#    fig = plt.figure(constrained_layout=True, figsize=(15,10) )
#    gs = gridspec.GridSpec(nrows=1, ncols=3, left=0.1, right=0.9, top=0.9, bottom=0.1, wspace=0.1, hspace=0.1)        
        
    
    fig, ax = plt.subplots(1,3, constrained_layout=True, figsize=(15,5), gridspec_kw={'width_ratios': [1,2,1]} )
...
#    ax4 = fig.add_subplot(gs[0, 0]); ax4.axis('on') 
#    ax5 = fig.add_subplot(gs[0, 1]); ax5.axis('on') 
#    ax6 = fig.add_subplot(gs[0, 2]); ax6.axis('on') 

    ax4.bar( x[:p5n], y[:p5n], color=colors[:p5n], width=barwidthfraction )
    ax5.bar( x[p5n:p5n+p6n], y[p5n:p5n+p6n], color=colors[p5n:p5n+p6n], width=barwidthfraction )
    ax6.bar( x[p5n+p6n:], y[p5n+p6n:], color=colors[p5n+p6n:], width=barwidthfraction )
...
#    ax4.set_ylim([ np.nanmin(y), np.nanmax(y)])
#    ax5.set_ylim([ np.nanmin(y), np.nanmax(y)])
#    ax6.set_ylim([ np.nanmin(y), np.nanmax(y)])

    ax4.set_ylim([ -0.75, 4.0])
    ax5.set_ylim([ -0.75, 4.0])
    ax6.set_ylim([ -0.75, 4.0])
...
#    ax3.xaxis.set_ticks( np.linspace(-10,-2,5) ); ax3.xaxis.set_ticklabels( np.linspace(-10,-2,5).astype(int) )
    ax4.xaxis.set_ticks( np.linspace(0,1800,10) ); ax4.xaxis.set_ticklabels( np.linspace(0,1800,10).astype(int) )
    ax5.xaxis.set_ticks( np.linspace(1850,2000,6) ); ax5.xaxis.set_ticklabels( np.linspace(1850,2000,6).astype(int) )
    ax6.xaxis.set_ticks( np.linspace(2020,2100,9) ); ax6.xaxis.set_ticklabels( np.linspace(2020,2100,9).astype(int) )
...
#    ax1.tick_params('both',length=7.5,width=2,which='major',color='lightgrey')             
...
#    ax3.tick_params('both',length=7.5,width=2,which='major',color='lightgrey')             
#    adjust_spines(ax3, ['bottom'])            
#    ax3.spines['bottom'].set_linewidth(2)
#    ax3.spines['bottom'].set_color('lightgrey')
    ...
    ax5.tick_params('both',length=7.5,width=2,which='major',color='lightgrey')             
    adjust_spines(ax5, ['bottom'])            
    ax5.spines['bottom'].set_linewidth(2)
    ax5.spines['bottom'].set_color('lightgrey')
...
#        cbar_ax = fig.add_axes([0.02, 0.75, 0.2, 0.02])
#        cbar = fig.colorbar(sm, cax=cbar_ax, shrink=0.5, extend='both', orientation='horizontal')
#        cbar.set_label( cbarstr, rotation=0, labelpad=0, fontsize=fontsize )    

        cbar_ax = fig.add_axes([0.05, 0.4, 0.01, 0.5])
        cbar = fig.colorbar(sm, cax=cbar_ax, shrink=0.5, extend='both', orientation='vertical')
        cbar.set_label( cbarstr, rotation=90, labelpad=10, fontsize=fontsize)    

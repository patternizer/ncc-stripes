...
p1x = np.linspace( p1start + p1unit/2., p1end - p1unit/2., p1n ) 
p1y = s1.statistic
...
y2 = df[ (df.Year >= -2.58e6) & (df.Year < -450.0e3) ]['Global']
x2 = df[ (df.Year >= -2.58e6) & (df.Year < -450.0e3) ]['Year']
s2 = stats.binned_statistic( x2, y2, 'mean', bins=np.linspace( p2start, p2end, p2n + 1 ) )
p2x = np.linspace( p2start + p2unit/2., p2end - p2unit/2., p2n ) 
p2y = s2.statistic
...
y3 = df[ (df.Year >= -478.0e3) & (df.Year < -11000) ]['Global']
x3 = df[ (df.Year >= -478.0e3) & (df.Year < -11000) ]['Year']
s3 = stats.binned_statistic( x3, y3, 'mean', bins=np.linspace( p3start, p3end, p3n + 1 ) )
p3x = np.linspace( p3start + p3unit/2, p3end - p3unit/2., p3n ) 
p3y = s3.statistic
...
p4x = np.linspace( p4start + p4unit/2., p4end - p4unit/2., p4n ) 
p4y = s4.statistic
...
y5 = df[ (df.Year >= 0) & (df.Year < 500) ]['Global']
x5 = df[ (df.Year >= 0) & (df.Year < 500) ]['Year']
s5 = stats.binned_statistic( x5, y5, 'mean', bins=np.linspace( p5start, p5end, p5n + 1) )
p5x = np.linspace( p5start + p5unit/2., p5end - p5unit/2., p5n ) 
p5y = s5.statistic
...
y6 = df[ (df.Year >= 500) & (df.Year < 1400) ]['Global']
x6 = df[ (df.Year >= 500) & (df.Year < 1400) ]['Year']
s6 = stats.binned_statistic( x6, y6, 'mean', bins=np.linspace( p6start, p6end, p6n + 1) )
p6x = np.linspace( p6start + p6unit/2., p6end - p6unit/2., p6n )
p6y = s6.statistic
...
# PANEL 3b:  1850 - 2020 CE ---------------------------------------------------

p8unit = 1e0
p8start = 1850
p8end = 2020
p8n = int( (p8end - p8start) / p8unit )

y8 = df[ (df.Year >= 1850) & (df.Year < 2020) ]['Global']
x8 = df[ (df.Year >= 1850) & (df.Year < 2020) ]['Year']
s8 = stats.binned_statistic( x8, y8, 'mean', bins=np.linspace( p8start, p8end, p8n + 1) )
p8x = np.linspace( p8start + p8unit/2., p8end - p8unit/2., p8n ) 
p8y = s8.statistic

# PANEL 4: 2020 - 2200 CE ---------------------------------------------------------

p9unit = 1e0
p9start = 2020
p9end = 2200
p9n = int( (p9end - p9start) / p9unit )

y9 = df[ (df.Year >= 2020) & (df.Year < 2200) ]['Global']
x9 = df[ (df.Year >= 2020) & (df.Year < 2200) ]['Year']
s9 = stats.binned_statistic( x9, y9, 'mean', bins=np.linspace( p9start, p9end, p9n + 1) )
p9x = np.linspace( p9start + p9unit/2., p9end - p9unit/2., p9n ) 
p9y = s9.statistic

x = np.array( list(p1x) + list(p2x) + list(p3x) + list(p4x) + list(p5x) + list(p6x) + list(p7x) + list(p8x) + list(p9x) )
y = np.array( list(p1y) + list(p2y) + list(p3y) + list(p4y) + list(p5y) + list(p6y) + list(p7y) + list(p8y) + list(p9y) )
...
if plot_climate_timeseries == True:
    ...
    figstr = 'climate-timeseries' + '-' + baselinestr + '-' + 'Panels_all' + '.png'
...
if plot_climate_bars == True:
    ...
    figstr = 'climate-bars' + '-' + baselinestr + '-' + 'Panels_all' + '.png'
...
if plot_climate_stripes == True:
    ...
    figstr = 'climate-stripes' + '-' + baselinestr + '-' + 'Panels_all' + '.png'
